...
char *processString_DragonCurve(char *prev)
{
	...
	{
		strcat(prev, applyRule_DragonCurve(prev[i]));
	}
	return prev;
}
...
char *createSystem_DragonCurve(int numIters, char *axiom)
{
	...
	{
		end = processString_DragonCurve(start);
		start = end;
	}
	printf("done w/ system\n");
}
...
void draw_DragonCurve(char *input, int old_x, int old_y)
{
	...
	for (i = 0; i < strlen(input); i++)
	{
		...
		else if (input[i] == 'F')
		{
			if (a % 360 == 0)
			{
				VGA_line(old_x, old_y, old_x, old_y - length, red);
				new_x = old_x;
				new_y = old_y - length;
			}
			else if (a % 270 == 0)
			{
				VGA_line(old_x, old_y, old_x - length, old_y, yellow);
				new_x = old_x - length;
				new_y = old_y;
			}
			else if (a % 180 == 0)
...
int main(void)
{
	...
	gettimeofday(&t1, NULL);
	char tree = createSystem_DragonCurve(5, "FX");
	draw_DragonCurve(tree, 300, 300);
	gettimeofday(&t2, NULL);
	elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000000.0; // sec to us
	elapsedTime += (t2.tv_usec - t1.tv_usec);					 // us to
	sprintf(time_string, "T=%8.0f uSec  ", elapsedTime);
